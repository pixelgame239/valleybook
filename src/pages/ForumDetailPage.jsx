...
const ForumDetailPage = () => {
  ...
  const handleReply = async () => {
    const counter =
      !topicData.reply || topicData.reply.length === 0
        ? 0
        : topicData.reply.length;
    setAnswerContent("");
  ...
  useEffect(() => {
    ...
    const forum_channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "forum", filter: `id=eq.${id}` },
        async (payload) => {
          await fetchTopic();
        }
      )
        ...
        <div>
          {showDelete ? (
            <DeleteForumPopup
              ...
              onDelete={async () => handleDeleteTopic(topicData.id)}
            ></DeleteForumPopup>
          ) : null}
          {showEdit ? (
            <EditForumPopup
          ...
          <div className="back-to-forum" onClick={() => navigate("/forum")}>
          ...
          <div className="forum-topic">
            <div className="topic-content">
              ...
              <p className="topic-text">{topicData.topic}</p>
              <p className="topic-username">
                Người đăng: {topicData.username}
                <span className="topic-date-created">
                  Ngày đăng: {new Date(topicData.created_at).toLocaleString()}
                </span>
              </p>
            </div>
            <div className="topic-emotion">
              {userInfo ? (
                <>
                  <span
                    className={`topic-like ${
                      topicData.like_count &&
                      topicData.like_count.includes(userInfo.email)
                        ? "active"
                        : ""
                    }`}
                    onClick={async () => await handleEmoChange("like_count")}
                  >
                    {!topicData.like_count || topicData.like_count.length === 0
                      ? 0
                      : topicData.like_count.length}
                    <ThumbUpAltIcon sx={{ fontSize: 40 }} />
                  </span>
                  <span
                    className={`topic-dislike ${
                      topicData.dislike_count &&
                      topicData.dislike_count.includes(userInfo.email)
                        ? "active"
                        : ""
                    }`}
                    onClick={async () => await handleEmoChange("dislike_count")}
                  >
                    {!topicData.dislike_count ||
                    topicData.dislike_count.length === 0
                      ? 0
                      : topicData.dislike_count.length}
                ...
                <>
                  ...
                  <span
                    className={`topic-dislike active`}
                    style={{ cursor: "default" }}
                  >
                    {!topicData.dislike_count ||
                    topicData.dislike_count.length === 0
            ...
            {userInfo &&
            (userInfo.username === topicData.username ||
              userInfo.username.endsWith("@valleybook.com")) ? (
              <div className="topic-interaction">
                <span
                  className="edit-topic"
                  onClick={() => setShowEdit((prev) => (prev = true))}
                >
                  <EditRoundedIcon sx={{ fontSize: 40 }}></EditRoundedIcon>
                </span>
                <span
                  className="delete-topic"
                  onClick={() => setShowDelete((prev) => (prev = true))}
                >
                  <DeleteRoundedIcon sx={{ fontSize: 40 }}></DeleteRoundedIcon>
                </span>
              </div>
            ) : null}
          </div>
